...
applications = repo.list_job_applications(conn, user["uid"])
...
for app in applications:
    ...
    eligibility = repo.get_latest_eligibility_result(conn, app["id"])
    match = repo.get_latest_match_result(conn, app["id"])
    versions = repo.list_resume_versions(conn, app["id"])
    ...
    with st.container(border=True):
        ...
        if versions:
            ...
            for v in versions:
                ...
                if v["pdf_file_path"] and Path(v["pdf_file_path"]).exists():
                    st.download_button(
                        f"Download {v['name']}.pdf", Path(v["pdf_file_path"]).read_bytes(),
